# Contrast Finance Backend v0.23
...
Добавлена первая защита endpoints по ролям.

Открытые:

```text
POST /auth/login
POST /auth/bootstrap-admin   # только пока нет active admin
```
...
Admin или руководитель отдела:

```text
GET /department-head-dashboard
```

Для `department_head` добавлена проверка: можно смотреть только свой `department_id`.
