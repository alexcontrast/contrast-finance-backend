# Contrast Finance Backend v0.31.6
...
Payment requests будут закрыты следующим шагом v0.31.6.1, чтобы не сломать рабочую очередь.
...
Скелет новой админки вкладками.
...
```text
общий план компании
линия выполнения плана
планы по отделам и заполнение
индивидуальные менеджеры и их планы/факт
```
...
фильтр по статусу
поиск по заказчику
клик по мероприятию открывает модальное окно со сметой, итогами и заявками
```

Заявки:

```text
поле Менеджер первым
потом Заказчик, Позиция, Сумма
фильтр по статусу
поиск по заказчику
```

Планы и Закрытие месяца пока как frontend-скелет.
Миграций нет, база не менялась.


## v0.31.6

Фирменное оформление web-админки в стиле Contrast.

Изменения:

```text
логотип Contrast в шапке
светло-зелёный фон
белые карточки
салатовые акценты
чёрный только как дозированный акцент
Санжар — голубой оттенок
Рауфаль — тёплый персиковый оттенок
таблица сметы в модалке с цветовым разделением групп столбцов
```

Миграций нет, backend-логика не менялась.


## v0.31.6

Переделана вкладка `Обзор` в стиле текущего сайта Contrast.

Теперь:

```text
сверху широкая карточка общего плана компании
крупный факт компании
ниже цель и процент выполнения
длинная линия заполнения
ниже две карточки отделов в 2 колонки
каждый отдел со своим цветом
внутри отдела менеджеры: факт, процент, шкала заполнения
```
...
Упрощён верх админки.

Изменения:

```text
убраны дублирующие верхние карточки:
  План компании / Факт компании / Выполнение / Расходы

выбор месяца заменён:
  было одно поле даты
  стало выпадающее меню Месяц + выпадающее меню Год
```

Вся финансовая информация остаётся во вкладке `Обзор`.
...
Исправлены заявки и UX загрузки.

Изменения:

```text
в заявках менеджер теперь подставляется даже если payment request не содержит manager_name:
  frontend берёт менеджера из связанного мероприятия по event_id

добавлен загрузочный слой:
...
добавлена вкладка Архив заявок:
  Отменено
  Деньги в кассе

вкладка Заявки теперь показывает активные заявки:
  Новая
  Оплачено

кнопка Возврат для Деньги в кассе сохранена
```
...
Исправлена модалка мероприятия и фильтр заявок внутри неё.

Изменения:

```text
в модалке мероприятия убран столбец Тип
...
координатор всегда первой строкой в смете
НДС/Вычеты показываются только для По счету, иначе 0
в заявках мероприятия убран поиск по заказчику
фильтр статуса заявок работает внутри модалки, без перезагрузки всей админки
```
...
Уточнено отображение вычетов в модалке мероприятия.

Правила отображения:

```text
По счету:
  НДС / Вычеты показываются по данным КГД/позиции
...
Если у самозанятого в старой тестовой позиции вычеты не записаны, frontend показывает 10% от факта, а если факта нет — от суммы по смете.
...
Hotfix cache для web-админки.

Проблема:
backend version обновлялся, но браузер продолжал использовать старый `/web/app.js`, поэтому модалка мероприятия визуально не менялась.
...
index.html подключает:
  /web/app.js?v=0.31.6
  /web/styles.css?v=0.31.6

web.py отдаёт index/app.js/styles.css с заголовками:
