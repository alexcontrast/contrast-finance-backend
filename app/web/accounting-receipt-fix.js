(() => {
  ...
  if (typeof previousParser === "function") {
    window.accountingParseESalyqText = (rawText, requestAmount = null) => {
      ...
      const iinMatch = String(rawText || "").match(/(?:ИИН|IIN|ЖСН)\s*[:№#-]*\s*((?:[0-9ОOоoІIil|][ \t-]?){12,18})/iu);
      if (iinMatch) {
        const normalized = iinMatch[1]
          .replace(/[ОOоo]/g, "0")
          .replace(/[ІIil|]/g, "1")
          .replace(/\D/g, "");
        if (normalized.length === 12) result.iin = normalized;
      }
      const numberMatch = String(rawText || "").match(/(?:Чек|Receipt)\s*(?:№|N|#|номер|number)?\s*[:#=-]*\s*((?:[0-9ОO][ \t-]?){4,24})/iu);
      if (numberMatch) {
        const normalized = numberMatch[1].replace(/[ОO]/g, "0").replace(/\D/g, "");
        if (normalized.length >= 4) result.receipt_number = normalized;
      }
      return result;
    };
  }
  ...
  function patchTesseract(instance) {
    if (!instance || instance.__contrastKazakhPatched) return instance;
    if (typeof instance.createWorker === "function") {
      const originalCreateWorker = instance.createWorker.bind(instance);
      instance.createWorker = async (languages, ...rest) => {
        const requested = String(languages || "");
        if (requested === "rus+eng") {
          try {
            return await originalCreateWorker("kaz+rus+eng", ...rest);
          } catch (error) {
            console.warn("Kazakh OCR worker unavailable, falling back to rus+eng", error);
            return originalCreateWorker("rus+eng", ...rest);
          }
        }
        return originalCreateWorker(languages, ...rest);
      };
    }
    if (typeof instance.recognize === "function") {
  ...
  console.info("Contrast Finance accounting receipt date/Kazakh fix v0.5.107 loaded");
})();
