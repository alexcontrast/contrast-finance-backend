...
def apply_accounting_receipt_data_fix() -> None:
    """Install v0.5.107 receipt parser fixes into the live accounting route.

    This intentionally patches the functions that the existing large v0.5.103
    route actually calls. QR/KGD stays authoritative; visual OCR is only a
    fallback for fields absent from the KGD report.
    """
    from app.api.routes import self_employed_accounting as accounting

    if getattr(accounting, "_receipt_data_fix_v0107", False):
        return
    ...
    def patched_clean_person_name(value: Any) -> str | None:
        return _unicode_clean_person_name(value)
    ...
    def patched_parse_esalyq_report_text(raw_text: str) -> dict[str, Any]:
        ...
        iin = _extract_iin_from_text(raw_text, str(accounting.R1_CUSTOMER_PROFILE.get("bin_iin") or ""))
        if iin:
            result["iin"] = iin
        receipt_number = _extract_receipt_number_from_text(raw_text)
        if receipt_number:
    ...
    def patched_parse_esalyq_json(payload: Any) -> dict[str, Any]:
        ...
        # Re-select legal fields instead of trusting generic create/id/number keys.
        ...
        if contractor:
            result["contractor_full_name"] = contractor
        iin = _extract_json_iin(accounting, payload)
        if iin:
            result["iin"] = iin
        receipt_number = _extract_json_receipt_number(accounting, payload)
        if receipt_number:
            result["receipt_number"] = receipt_number
        return result
    ...
    def patched_build_row(*args: Any, **kwargs: Any):
        ...
        # A missing legal receipt date must never be displayed as upload/today.
    ...
    accounting.clean_person_name = patched_clean_person_name
    ...
    accounting._receipt_data_fix_v0107 = True
