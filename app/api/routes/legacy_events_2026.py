...
from app.services.legacy_events_2026_importer import dry_run_legacy_events_2026_xlsx, import_legacy_events_2026_xlsx
...
<head>
  ...
  <title>Contrast legacy events 2026 import</title>
  <style>
...
<body>
  <div class="card">
    <h1>Импорт мероприятий 2026</h1>
    <p class="muted"><b>Страница v0.5.12</b>. Читает только листы Январь–Апрель 2026. Dry-run базу не меняет, боевой импорт создаёт черновики мероприятий и строки сметы.</p>
    <p class="warn">Оплаты, заявки и Telegram не импортируются. Боевой импорт загрузит исторические мероприятия как <b>черновики</b> на менеджера <b>Тест</b> и пропустит уже импортированные события, чтобы не создать дубли.</p>
    ...
    <div class="row"><button id="dryBtn" type="button">Запустить dry-run</button><button id="importBtn" type="button" style="background:#111;color:#8DFF00">Импортировать в базу</button><button id="clearBtn" type="button">Очистить отчёт</button></div>
    <p class="muted">Перед импортом страница попросит подтверждение: <b>IMPORT 2026 TO TEST</b>.</p>
...
(function(){
  ...
  function render(data){
    ...
    var html = '<h2>Итог</h2>';
    html += manager.found === false ? '<p class="bad">Менеджер "'+esc(manager.requested_name)+'" не найден в базе.</p>' : '<p class="ok">'+(data.dry_run ? 'Dry-run: база не менялась.' : 'Импорт выполнен.')+' Менеджер: '+esc(manager.name || manager.requested_name)+(manager.found ? ' найден' : '')+'.</p>';
    if(data.stats){ html += '<div class="grid">'+metric('Создано мероприятий', data.stats.events_created)+metric('Пропущено дублей', data.stats.events_skipped_existing)+metric('Создано строк сметы', data.stats.items_created)+metric('Создано заявок', data.stats.payment_requests_created)+'</div>'; }
  ...
  async function runLegacy(mode){
    ...
    if(mode === 'import'){
      var confirmation = prompt('Боевой импорт создаст мероприятия и строки сметы на менеджера "'+manager+'". Оплаты/заявки/Telegram не создаются. Для подтверждения введи: IMPORT 2026 TO TEST');
      if(confirmation !== 'IMPORT 2026 TO TEST'){
        setStatus('Импорт отменён: подтверждение не совпало.', 'status bad');
        return;
      }
    }
    var btn = mode === 'import' ? byId('importBtn') : byId('dryBtn');
    byId('dryBtn').disabled = true; byId('importBtn').disabled = true;
    setStatus('Файл выбран: <b>'+esc(file.name)+'</b> ('+fmt(file.size)+' байт). '+(mode === 'import' ? 'Импортирую в базу…' : 'Отправляю dry-run на backend…'), 'status');
    try{
      var form = new FormData();
      form.append('token', token);
      form.append('target_manager_name', manager);
      form.append('file', file, file.name);
      var url = mode === 'import' ? '/api/legacy-events-2026/import' : '/api/legacy-events-2026/dry-run';
      var res = await fetch(url, { method:'POST', body: form, cache:'no-store' });
      var text = await res.text();
      var data;
      try { data = JSON.parse(text); } catch(parseErr) { throw new Error('Backend вернул не JSON: HTTP '+res.status+' '+text.slice(0, 1000)); }
      if(!res.ok || data.ok === false){ setStatus('<pre>'+esc(JSON.stringify(data,null,2))+'</pre>', 'status bad'); return; }
      render(data);
    } catch(err){
      setStatus('<pre>'+esc(err && err.stack ? err.stack : String(err))+'</pre>', 'status bad');
    } finally {
      byId('dryBtn').disabled = false; byId('importBtn').disabled = false;
    }
  }
  ...
  byId('dryBtn').addEventListener('click', function(e){ e.preventDefault(); runLegacy('dry-run'); });
  byId('importBtn').addEventListener('click', function(e){ e.preventDefault(); runLegacy('import'); });
...
@router.post("/api/legacy-events-2026/import")
async def legacy_events_2026_import(
    token: str = Form(...),
    target_manager_name: str = Form("Тест"),
    file: UploadFile = File(...),
    db: Session = Depends(get_db),
    ...
    if not raw:
        raise HTTPException(status_code=400, detail="Файл пустой")
    return import_legacy_events_2026_xlsx(raw, target_manager_name=target_manager_name, db=db, imported_by="legacy-events-2026-page")
